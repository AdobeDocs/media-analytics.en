...
1. **Call [Bitrate change](bitrate-change.md)** whenever the player negotiates a new bitrate. Include the current QoE data — bitrate, frames per second, dropped frames — so the backend can compute [Average bitrate](/help/reporting/metrics/average-bitrate.md) and related quality metrics.
