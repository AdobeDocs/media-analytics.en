...
This page covers the Analytics-only JavaScript 3.x SDK. For the recommended implementation, see [Implement Streaming Media using the Edge Network](/help/implementation/edge/edge-web-sdk.md).
...
`ADB.Media.VideoMetadataKeys`, `ADB.Media.AudioMetadataKeys`, and `ADB.Media.AdMetadataKeys` provide the context data key strings for standard metadata. For the full list of keys and their corresponding reporting variables, see the [Standard metadata variable reference](/help/implementation/variables/standard-metadata/show.md).
