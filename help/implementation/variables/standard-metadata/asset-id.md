...
| **XDM collection field** | [`xdm.mediaCollection.sessionDetails.assetID`](https://experienceleague.adobe.com/en/docs/experience-platform/xdm/data-types/session-details-collection) |
...
Set `assetID` inside `xdm.mediaCollection.sessionDetails` when calling [`sendEvent`](https://experienceleague.adobe.com/en/docs/experience-platform/collection/js/commands/sendevent/overview):
...
Call the [sessionStart](https://developer.adobe.com/data-collection-apis/docs/endpoints/media/sessions/#sessionstart) endpoint with `assetID` inside `xdm.mediaCollection.sessionDetails`:
...
Use `MEDIA_VideoMetadataKeyASSET_ID` to set the asset ID in the standard metadata of the media object before calling `mediaTrackSessionStart`:
