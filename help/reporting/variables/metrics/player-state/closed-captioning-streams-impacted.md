...
The media backend sets the `isSet` flag in `mediaReporting.states[]` for the `closedCaptioning` entry to `true` the first time a `media.statesUpdate` event with `closedCaptioning` in `statesStart` is received. The metric is reported on the close call.
...
| Data feeds | `videostateclosedcaptioning` |
