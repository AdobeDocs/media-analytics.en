...
The media backend sets this flag when `xdm.mediaCollection.sessionDetails.hasResume` is `true` on the [session start](/help/implementation/events/session/session-start.md) event. The player must explicitly flag the session as a resume. The metric is reported on the close call.
