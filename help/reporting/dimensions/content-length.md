...
The **Content length** dimension reports the total duration in seconds of each media session as set at session start. It powers backend metrics including [progress markers](/help/reporting/metrics/progress-markers.md) and [Average Minute Audience](/help/reporting/metrics/average-minute-audience.md).
...
| --- | --- |
| Adobe Analytics | Automatically collected from context data `a.media.length` when [[!UICONTROL Media Core]](/help/reporting/setup/analytics-reporting.md) is enabled. |
...
>[!NOTE]
>
>In Adobe Analytics, this value also corresponds to a **Video length** classification on the [Content](content.md) dimension. You are responsible for populating and maintaining that classification separately. Customer Journey Analytics uses this dimension directly. You can use [Value bucketing](https://experienceleague.adobe.com/en/docs/analytics-platform/using/cja-dataviews/component-settings/value-bucketing) if desired.
